<%@page import="kr.or.ddit.prod.model.PageVo"%>
	...
	<div class="col-sm-8 blog-main">
		...
		<div class="table-responsive">
			<table class="table table-striped">
				...
				<%System.out.println(request.getAttribute("prodList")); %>
					
					<c:forEach items="${prodList }" var="pl" varStatus="status">
						<tr class="userClick">
							<td>${pl.rnum}</td>
							<td><a href="/prodDetail?prod_id=${pl.prod_id}">${pl.prod_id}</a></td>
							<td>${pl.prod_name }</td>
							<td>${pl.lprod_nm }</td>
						<td><fmt:formatDate value="${pl.prod_insdate}" pattern="yyyy-MM-dd" /></td>
					</tr>	
					</c:forEach>
		...
		<div class="text-center">
			<ul class="pagination">
			<li><a href="/prodList?pageNum=<%=moveFoward%>"><</a></li>
			<%
			for(int pg = startIndex;pg < endIndex;pg++){
				if(pg==pageVo.getPage()){
					%><li><a href="/prodList?pageNum=<%=pg%>" style="background:#428bca; color:white;"><%=pg%></a></li><%
				} else{
				%>
				<li><a href="/prodList?pageNum=<%=pg%>"><%=pg%></a></li>
				<%
				}
			}
			%>
			...
			<li><a href="/prodList?pageNum=<%=endIndex%>">></a></li>
			</ul>
		</div>
	</div>
</div>
